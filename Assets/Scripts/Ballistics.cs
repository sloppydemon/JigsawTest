...
public class Ballistics : MonoBehaviour
{
    ...
    public static int SolveQuartic(double c0, double c1, double c2, double c3, double c4, out double s0, out double s1, out double s2, out double s3)
    {
        ...
        else
        {
            ...
            else if (num == 1) num += Ballistics.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s1, out s2);
            else if (num == 2) num += Ballistics.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s2, out s3);
        }
    ...
    public static float BallisticRange(float speed, float gravity, float initial_height)
    {
        ...
        Debug.Assert(speed > 0 && gravity > 0 && initial_height >= 0, "Ballistics.CalculateRange called with invalid data");
    ...
    public static int SolveBallisticArc(Vector3 proj_pos, float proj_speed, Vector3 target, float gravity, out Vector3 s0, out Vector3 s1)
    {
        ...
        Debug.Assert(proj_pos != target && proj_speed > 0 && gravity > 0, "Ballistics.CalculateVector called with invalid data");
